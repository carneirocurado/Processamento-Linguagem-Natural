...
QTDE_FILES=$(ls -1 ./corpus2 | wc -l)
qtde_processada=0

for arquivo in ./corpus2/*; do
...
#				fi
				...
				time ./word2vec/word2vec -train $arquivo -output $ARQ_WORDSPACE -cbow 1 -size 200 -window $i -negative 25 -threads 20 -binary 1 -iter 15
				...
				printf "\n------------------------------\n%s: Iniciando Construcao do Wordspace: %s\n" "$(timestamp)" "$ARQ_WORDSPACE"
				time ./word2vec/word2vec -train $arquivo -output $ARQ_WORDSPACE -cbow 0 -size 200 -window $i -negative 25 -threads 20 -binary 1 -iter 15
				printf "\n\n%s: Iniciando Analise: %s\n" "$(timestamp)" "$ARQ_WORDSPACE"
				./word-analogy2 $ARQ_WORDSPACE $ARQ_SAIDA < $ARQ_IN
			done
		fi
